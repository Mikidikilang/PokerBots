...
def _card_to_treys_str(card: str) -> str:
    ...
    A belső formátum: "RS" (pl. "AS" = Ász Pikk)
    A Treys formátum: "Rs" (pl. "As" = Ász Pikk, kisbetűs szín)

    Args:
        card: Kétkarakteres kártyajelölés belső formátumban.
...
def _build_full_deck() -> list[str]:
    """Felépíti a teljes 52 lapos paklit a belső formátumban.

    Returns:
        52 elemű lista kétkarakteres kártyajelölésekkel.
    """
...
class EquityCalculator:
    ...
    def calculate_equity(
        ...
        Args:
            hole_cards: Saját 2 lap (pl. ["AS", "KH"]).
    ...
    def _evaluate_simple(
        ...
        for card in all_cards:
            card = card.strip().upper()
            if len(card) >= 2 and card[0] in rank_order:
                ranks.append(rank_order.index(card[0]))
                suits.append(card[1])
    ...
    def get_preflop_hand_category(hole_cards: list[str]) -> str:
        ...
        Args:
            hole_cards: A 2 saját lap (pl. ["AS", "KH"]).
        ...
        r1_idx: int = rank_order.index(c1[0]) if c1[0] in rank_order else -1
        r2_idx: int = rank_order.index(c2[0]) if c2[0] in rank_order else -1
        suited: bool = c1[1] == c2[1]
