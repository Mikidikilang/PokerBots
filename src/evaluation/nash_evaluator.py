...
class LocalBestResponseEvaluator:
    ...
    def _model_step(self, obs_dict: dict[str, Any]) -> dict[str, Any] | None:
        ...
        try:
            ...
            # Flatten observation into single tensor (required by networks)
            flat_obs = self.obs_builder.flatten(obs_tensors)  # (obs_dim,)
            batched_obs = flat_obs.unsqueeze(0).to(self.device)  # (batch=1, obs_dim)
    ...
    def _oracle_best_response_ev(
        ...
        try:
            ...
            # Flatten observation into single tensor (required by networks)
            flat_obs = self.obs_builder.flatten(obs_dict)  # (obs_dim,)
            batched_obs = flat_obs.unsqueeze(0).to(self.device)  # (batch=1, obs_dim)
            ...
            with torch.inference_mode():
                opponent_logits = self.strategy_network(batched_obs)  # (batch=1, 12)
